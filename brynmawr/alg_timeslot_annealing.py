import math
import random
# assign with different classArr
def calculate_score(classArr, stuArr, classroomArr, class_ID_real, student_ID_real, timeArr):
    score = 0
    classArray = classArr.copy()
        ...
        for j in range(10):
            if(stu[j] != -1):
                if stu[j] in class_ID_real:
                    if(classArray[class_ID_real[stu[j]]].aval > 0):
                        ...
                        if can:
                            ...
                            score += 1
                            if stu[j] in done:
                                classArray[class_ID_real[stu[j]]].students = f"{classArray[class_ID_real[stu[j]]].students} {student_ID_real[i]}"
                            else:
                                classArray[class_ID_real[stu[j]]].students = f"{student_ID_real[i]}"
                            classArray[class_ID_real[stu[j]]].aval = classArray[class_ID_real[stu[j]]].aval - 1
                    done.append(stu[j])
        i = i + 1
    return score

def replace(classArray, stuArr, classroomArr, class_ID_real, student_ID_real, timeArr):
    score = 0
    ...
    for stu in stuArr:
        ...
        for j in range(10):
            if(stu[j] != -1):
                if stu[j] in class_ID_real:
                    classArray[class_ID_real[stu[j]]].students = ""
                    # classArray[class_ID_real[stu[j]]].aval
        i = i + 1
    return classArray


# Neighbor function: generate a new arrangement by swapping two indices
def get_neighbor(classArr, index_change, class_ID_real, stuArr, classroomArr, student_ID_real, timeArr):
    neighbor = classArr.copy()
    length = len(index_change)
    # print(length)
    # Randomly choose two indices from indices_to_change
    i, j = random.sample(range(length), 2)
    # print(f"random:{i},{j}") checked
    i = index_change[i]
    j = index_change[j]
    classArr[index_change[i]], classArr[index_change[j]] = classArr[j], classArr[i] 
    total_score = calculate_score(classArr, stuArr, classroomArr, class_ID_real, student_ID_real, timeArr)

    return classArr, total_score

# Simulated Annealing function
def simulated_annealing(score_initial, indexArr, classArr, n_iterations, temp, class_ID_real, stuArr, classroomArr, student_ID_real, timeArr):
    # Initial solution
    current = classArr[:]
    best = current[:]
    best_eval = score_initial
    ...
    for i in range(n_iterations):
        ...
        candidate, candidate_eval = get_neighbor(classArr, indexArr, class_ID_real, stuArr, classroomArr, student_ID_real, timeArr)
        # print(candidate)
        ...
        if candidate_eval > best_eval or random.random() < math.exp((candidate_eval - current_eval) / t):
            ...
            print(best_eval)
            if candidate_eval > best_eval:  # We maximize the score
                print(f"candidate: {candidate_eval}")
                best, best_eval = candidate, candidate_eval
                scores.append(best_eval)
    print(best_eval)
    return best, best_eval, scores

def annealing(arr, classArr, class_ID_real, stuArr, score_inital, classroomArr, student_ID_real, timeArr):
    # Example array of integers
    random.shuffle(arr)  # Start with a random arrangement
    # print(f"Initial arrangement: {arr}")

    n_iterations = 1000 # larger
    temp = 10

    # Perform the simulated annealing search
    best_arrangement, best_score, scores = simulated_annealing(score_inital, arr, classArr, n_iterations, temp, class_ID_real, stuArr, classroomArr, student_ID_real, timeArr)

    # print(f'Best Arrangement: {best_arrangement}')
    # print(f'Best Score: {best_score}')
    return best_arrangement, best_score

def conflict_set(classArr, stuArr, classroomArr, class_ID_real, student_ID_real, timeArr):
    score = 0
    conflictArr = []
    for a in classArr:
        a.aval = classroomArr[a.classroom]
                ...
                if stu[j] in class_ID_real:
                    if(classArr[class_ID_real[stu[j]]].aval > 0):
                        ...
                        for occu in occu_times:
                            ...
                            if if_conflict(timeArr[int(occu)], timeArr[int(timeslot)]):
                                can = False
                                if classArr[class_ID_real[stu[j]]] not in conflictArr:
                                    conflictArr.append(class_ID_real[stu[j]])
                                break
...
def main():
    ...
    """for i, a in enumerate(result):
        ...
        print(a.classroom)"""
    conflict = conflict_set(result, stuArr, classroomArr, class_ID_real, student_ID_real, timeArr)
    # print(len(conflict))
    # for i in conflict:
    #     print(i.real_ID)
    score = calculate_score(result, stuArr, classroomArr, class_ID_real, student_ID_real, timeArr)
    result, score = annealing(conflict, classArr, class_ID_real, stuArr, score, classroomArr, student_ID_real, timeArr)
    # for i in result:
    #     print(f"{i.real_ID} + {i.classroom}")
    # result = replace(result, stuArr, classroomArr, class_ID_real, student_ID_real, timeArr)
