...
def main():
    ...
    conflict_matrix = array = [[[-1] * classNum] * classNum]
    
    for stu in stuArr:
        for i in stu:
            if i != -1 or i is not "-1":
                for j in stu:
                    if j != -1 or j is not "-1":
                        if conflict_matrix[int(i)][int(j)] == -1:
                            conflict_matrix[int(i)][int(j)] = 1
                        else:
                            conflict_matrix[int(i)][int(j)] += 1
                        if conflict_matrix[int(j)][int(i)] == -1:
                            conflict_matrix[int(j)][int(i)] = 1
                        else:
                            conflict_matrix[int(j)][int(i)] += 1
    conflict_matrix = np.array(conflict_matrix)
    max_index = np.unravel_index(np.argmax(conflict_matrix), conflict_matrix.shape)
    print(max_index)
