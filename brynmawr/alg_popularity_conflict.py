...
def main():
    ...
    while True:
        ...
        if (conflict_matrix[max_index][max_index2] == 0) or (conflict_matrix[max_index][max_index2] == -999999) or (conflict_matrix[max_index][max_index2] == -1):
            break
        # print(classArr[class_ID_realmax_index].real_ID)
    ...
    popu_prpt = 0.1
    con_prpt = 1 - popu_prpt
    popu_weights = {id: (len(popuList) - index) * (popu_prpt) for index, id in enumerate(popuList)}
    conflict_weights = {id: (len(conflictList) - index) * (con_prpt) for index, id in enumerate(conflictList)}
    class_weights = {}
    for id in popuList:
        popu_weight = popu_weights.get(id, 0)
        conflict_weight = conflict_weights.get(id, 0)
        class_weights[id] = popu_weight + conflict_weight
    popconList = sorted(class_weights.keys(), key=lambda x: class_weights[x], reverse=True)
    ...
    result = schedule_classes(classArr, classroomArr, timeNum, roomList, popconList, profArr, class_ID_real, prof_ID_real, timeArr)
